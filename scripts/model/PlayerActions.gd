...
static func check_reshuffle(player: Player) -> bool:
    return player.state.current == StateManager.EState.RESHUFFLE and player.reshuffle_attempts > 0


static func check_validate_hand(player: Player) -> bool:
    return player.state.current == StateManager.EState.RESHUFFLE and not player.state.hand_ready


static func check_init_battlefield(player: Player) -> bool:
    return player.state.current == StateManager.EState.INIT_BATTLEFIELD and not player.state.battlefield_ready


static func check_init_reserve(player: Player) -> bool:
